...
# Vision-capable model. Swap for any vision model on your OpenRouter plan.
...
def _normalize_payload(payload) -> dict:
    ...
    raw_dates    = _find_key(payload, 'Exam_dates', 'exam_dates',
                             'ExamDates', 'Exam dates', 'examDates') or {}
    ...
    if isinstance(raw_subjects, list):
        for s in raw_subjects:
            ...
            if isinstance(topics, list):
                topics = {str(t).strip(): 'none' for t in topics
...
def _ensure_study_days(payload: dict, today_str: str) -> dict:
    """Build study_days from today_str → last exam date."""
    exam_dates  = payload.get('Exam_dates') or {}
    valid_dates = []
    ...
    payload['study_days'] = _date_range_inclusive(today_str, _format_date(last_exam))
    return payload
...
def organize_with_llm(file_paths: list,
    ...
    """
    file_paths  : saved file paths; syllabus file(s) first, datesheet LAST.
    manual_text : optional plain text pasted by the user in the browser.
    today_str   : DD-MM-YYYY string for study_days start.
                  If None, falls back to real datetime.date.today().
    """
...
"""})
    ...
    try:
        ...
        normalized = _ensure_study_days(normalized, today_str)
        return normalized
