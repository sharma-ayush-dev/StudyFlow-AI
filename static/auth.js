/* ═══════════════════════════════════════════
   ...
   On success: reload landing page so the navbar
   updates to show the logged-in state WITHOUT
   redirecting to /upload_page.
═══════════════════════════════════════════ */
...
// Buttons that exist on the page — guard with ?. since logged-in pages don't render them
...
async function authFetch(url, body, errorEl, btnEl, btnLabel) {
    ...
    try {
        ...
        // Stay on landing — just reload so Jinja re-renders with logged-in nav
...
registerSubmit?.addEventListener('click', () => {
    ...
    authFetch('/register', { username, email, password }, registerError, registerSubmit, 'Create Account');
});
