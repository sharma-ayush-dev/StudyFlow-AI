...
def otp_verify():
    ...
    if user:
        ...
        user = User(username=temp_username, email=email, course=None, session_version=0)
        import secrets
...
@auth_bp.route('/google/mock', endpoint='google_mock')
def google_mock():
    from flask import render_template
    return render_template('MockGoogleSignIn.html')
